<?php include(ROOT_DIRECTORY.'/app/views/header.php'); ?>
 ...
 <div class="row">
  ...
  <div class="row">
    <div class="col-md-12">
      <a target="_blank" href="<?= DIRECTORY_NAME ?>/export/synthese/personnalise" class="btn btn-primary exportButto">EXPORTER</a>
      <a target="_blank" href="<?= DIRECTORY_NAME ?>/pdf/imprimerSynthesePersonnalisee" class="btn btn-primary exportButto">IMPRIMER</a>
      <form method = "POST" action = "<?= DIRECTORY_NAME ?>/syntheses/pardate" id="parDateForm">
...
<div class="bs-example">
  <div class="table-responsive">
   <table class="table table-bordered table-hover table-transactions" id="journaliere">
      <thead>
        <tr><th colspan="6">SYNTHESE PERSONNALISEE</th><th colspan="2">Due le <?= $_SESSION['pardate']['from'] ?>  : <?= (!empty($data['transactions'][0]["balance_from"])) ?  number_format($data['transactions'][0]["balance_from"], 2, '.', " ") : "0.00" ?> HTG</th></tr>
        <tr>
          <th>Numéro</th>
          <th>Client</th>
          <th>Description</th>
      ...
      <tbody>
        <?php 
          ...
          for($i=0;$i<count($data['transactions']);$i++)
          {
            ...
            echo "<td class='editCat description'>" . $data['transactions'][$i]['client_firstname'] . " " . $data['transactions'][$i]['client_lastname'] . "</td>";
            if($data['transactions'][$i]['description'] == null)
            {
              echo "<td class='editCat description tdcenter'><strong>-</strong></td>";
            }
            else
            {
      ...
      <tfoot>
        ...
        <tr><th colspan="6"></th><th colspan="2">Due le <?= $_SESSION['pardate']['to'] ?> : <?= (!empty($data['transactions'][0]["balance_to"])) ?  number_format($data['transactions'][0]["balance_to"], 2, '.', " ") : "0.00" ?> HTG</th></tr>
      </tfoot>
    </table>
  </div>
