<?php include(ROOT_DIRECTORY.'/app/views/header.php'); ?>
 ...
 <div class="row">
  ...
  <div class="row">
    <div class="col-md-12">
      <a target="_blank" href="<?= DIRECTORY_NAME ?>/export/synthese/journaliere" class="btn btn-primary exportButto">EXPORTER</a>
      <form method = "POST" action = "<?= DIRECTORY_NAME ?>/syntheses" id="dailyForm">
        <input type="text" class="form-control dateInput dailydatepicker dailyInput" name="today" id='today' value = "<?= (!empty($_SESSION['daily'])) ? $_SESSION['daily'] : '' ?>" placeholder = "MM/JJ/AAAA">
      </form>
    </div>
  </div>
<div class="bs-example">
  <div class="table-responsive">
    <table class="table table-bordered table-hover table-transactions" id="journaliere">
      <thead>
        <tr><th colspan="6">SYNTHESE JOURNALIERE</th><th colspan="2">Balance matin  : <?= (!empty($data['transactions'][0]["balance_from"])) ?  number_format($data['transactions'][0]["balance_from"], 2, '.', " ") : "0.00" ?> HTG</th></tr>
        <tr>
          <th>Numéro</th>
          <th>Client</th>
          <th>Description</th>
      ...
      <tfoot>
        ...
        <tr><th colspan="6"></th><th colspan="2">Balance soir : <?= (!empty($data['transactions'][0]["balance_to"])) ?  number_format($data['transactions'][0]["balance_to"], 2, '.', " ") : "0.00" ?> HTG</th></tr>
      </tfoot>
    </table>
  </div>
